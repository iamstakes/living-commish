    ...
    var body: some View {
        Group {
            if isTakingQuiz, let quiz = player.quiz {
                ...
                CollegeFootballPlayerGalleryView(
                    player: player,
...
private struct CollegeFootballPlayerGalleryView: View {
    let player: CollegeFootballPlayerCard
    let isQuizRewardCollected: Bool
    ...
    var body: some View {
        ZStack {
            ...
            if let selectedImage {
                CollegeFootballGalleryRemoteImage(urlString: selectedImage.imageURL)
                    .scaleEffect(1.12)
                    .blur(radius: 42)
                    .opacity(0.20)
                    .ignoresSafeArea()
    ...
    private var galleryHeader: some View {
        HStack(spacing: 12) {
            ...
            .accessibilityLabel("Close Travis Hunter gallery")
            .accessibilityIdentifier("player-gallery-close")

            VStack(alignment: .leading, spacing: 2) {
                Text("THE TWO-WAY ARCHIVE")
                    .font(.system(size: 11, weight: .black))
                    .tracking(1.4)
                    .foregroundStyle(buffsGold)
    ...
    private var gallery: some View {
        TabView(selection: $selectedImageID) {
            ForEach(player.gallery) { galleryImage in
                CollegeFootballGalleryPhotoCard(image: galleryImage)
        ...
        HStack(spacing: 9) {
            ForEach(player.gallery) { galleryImage in
                Button {
                    ...
                    CollegeFootballGalleryRemoteImage(
            ...
            Button(action: onStartQuiz) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text(
                            isQuizRewardCollected
                                ? "Replay the Travis Hunter Quiz"
                                : "Take the Travis Hunter Quiz"
                        )
                        .font(.headline.weight(.black))
                        ...
                        Text(
                            isQuizRewardCollected
                                ...
                                : "Earn a LEGENDARY Travis Hunter card"
                        )
                ...
                .background(
                    LinearGradient(
                        colors: [
                            buffsGold,
                            Color(red: 0.39, green: 0.30, blue: 0.09),
                        ],
                        startPoint: .leading,
                        endPoint: .trailing
                    ),
    ...
    private var selectedImage: CollegeFootballPlayerGalleryImage? {
...
private struct CollegeFootballGalleryPhotoCard: View {
    let image: CollegeFootballPlayerGalleryImage

    var body: some View {
        ZStack(alignment: .bottomLeading) {
            CollegeFootballGalleryRemoteImage(urlString: image.imageURL)
...
private struct CollegeFootballGalleryRemoteImage: View {
