...
struct CollegeFootballSearchHomeView: View {
    var onProfileTap: () -> Void = {}
    var onCollectionTap: (CollegeFootballSticker) -> Void = { _ in }

    @Environment(CollegeFootballSearchEnvironment.self) private var environment
    @Environment(\.scenePhase) private var scenePhase
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    @Environment(\.openURL) private var openURL
    @State private var presentedDailyDrop: CollegeFootballDailyDrop?
    ...
    private var discoverySection: some View {
        ...
        .accessibilityIdentifier("collegeFootball-discovery-section")
    }

    private var activeDiscoveryCard: CollegeFootballDiscoveryCard? {
    ...
    private func resultsSection(
        _ experience: CollegeFootballSearchExperience
    ) -> some View {
...
struct CollegeFootballSearchControl: View {
    @Bindable var environment: CollegeFootballSearchEnvironment
    let accent: Color
        ...
        GlassEffectContainer(spacing: 10) {
            HStack(spacing: 10) {
                HStack(spacing: 10) {
                    ...
                    TextField(
                        "Ask me anything college football!",
                    ...
                    .accessibilityIdentifier("collegeFootball-search-field")
                    ...
                    if !environment.searchText.isEmpty {
                        ...
                        .accessibilityIdentifier("clear-collegeFootball-search")
                    }
                }
                ...
                .accessibilityLabel("Run college football search")
                .accessibilityIdentifier("collegeFootball-search-button")
            }
        }
        .accessibilityElement(children: .contain)
        .accessibilityIdentifier("collegeFootball-search-section")
...
private enum CollegeFootballHostStageLayout {
...
struct HostPresentationStage<Content: View>: View {
    ...
    init(
        ...
        hostAccessibilityHint: String = "Open your college football profile",
        @ViewBuilder content: () -> Content
    ) {
        self.host = host
        self.accent = accent
    ...
    var body: some View {
        GeometryReader { proxy in
            ZStack(alignment: .bottomTrailing) {
                ...
                AnimatedHostView(
                    host: host,
                    height: CollegeFootballHostStageLayout.hostHeight,
                    accent: accent,
                    contentScale: CollegeFootballHostStageLayout.hostScale,
                ...
                .frame(
                    maxWidth: .infinity,
                    maxHeight: .infinity,
                    alignment: CollegeFootballHostStageLayout.hostAlignment
                )
                .offset(
                    x: proxy.size.width
                        * CollegeFootballHostStageLayout.hostXOffsetFraction,
                    y: CollegeFootballHostStageLayout.hostYOffset
                )
                .contentShape(Rectangle())
                ...
                if let thought,
                   !thought.trimmingCharacters(
                    ...
                    .frame(
                        width: min(
                            proxy.size.width
                                * CollegeFootballHostStageLayout.thoughtWidthFraction,
                            CollegeFootballHostStageLayout.thoughtMaximumWidth
                        ),
                        height: CollegeFootballHostStageLayout.thoughtHeight,
                        alignment: .bottomLeading
                    )
                    .padding(
                        .leading,
                        CollegeFootballHostStageLayout.thoughtLeadingPadding
                    )
                    .padding(
                        .top,
                        CollegeFootballHostStageLayout.thoughtTopPadding
                    )
                    .frame(
                        maxWidth: .infinity,
                        maxHeight: .infinity,
                ...
                content
                    .frame(
                        maxWidth: .infinity,
                        maxHeight: .infinity,
                        alignment: CollegeFootballHostStageLayout.contentAlignment
                    )
                    .zIndex(2)

                HostReadyBadge(host: host)
                    .padding(.top, CollegeFootballHostStageLayout.badgeTopPadding)
                    .padding(.horizontal, 12)
                    .frame(
                        maxWidth: .infinity,
                        maxHeight: .infinity,
                        alignment: CollegeFootballHostStageLayout.badgeAlignment
                    )
                    .zIndex(3)
            }
        }
        .frame(height: CollegeFootballHostStageLayout.height)
        .accessibilityElement(children: .contain)
...
private struct DiscoveryPresentationDeck: View {
    ...
    let card: CollegeFootballDiscoveryCard
    let accent: Color
    let position: Int
    let count: Int
                ...
                Button(action: onOpen) {
                    Group {
                        if let dailyDrop = card.dailyDrop {
                            CollegeFootballDailyDropDiscoveryCardContent(
                                drop: dailyDrop,
    ...
    private var accessibilityLabel: String {
        if let score = card.finalScore {
            return """
            Final. \(score.visitorTeam) \(score.visitorPoints), \
            \(score.homeTeam) \(score.homePoints). \
            \(score.winningLeader), \(score.winningLeaderLine). \
            \(score.losingLeader), \(score.losingLeaderLine).
            """
        }
        ...
        if let standings = card.standings {
            ...
            \(standings.record), \(standings.conferenceRecord) in conference. \
            They are \(standings.rankedRecord) against ranked teams with a \
            \(standings.streak) streak. Their point differential is \
            \(standings.pointDifferential); only the \
            \(standings.comparisonTeam) are worse at \
            \(standings.comparisonPointDifferential). Next: \
    ...
    private var accessibilityHint: String {
        if card.dailyDrop != nil {
            return "Start the daily college football story and quiz"
        }
...
struct SearchResultPresentationDeck: View {
    ...
    let experience: CollegeFootballSearchExperience
    let reduceMotion: Bool
    let onOpenPlayerGallery: (CollegeFootballPlayerCard) -> Void
    ...
    var body: some View {
        ...
        .accessibilityElement(children: .contain)
        .accessibilityIdentifier("collegeFootball-results-deck")
    }

    private var resultModules: [CollegeFootballResultModule] {
    ...
    private var activeModule: CollegeFootballResultModule? {
...
private struct FinalScoreDiscoveryCardContent: View {
    let score: CollegeFootballFinalScoreSnapshot
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 7) {
                FinalScoreTeamRow(
                    ...
                    points: score.visitorPoints,
                    firstDowns: score.visitorFirstDowns,
                    turnovers: score.visitorTurnovers,
                    isHomeTeam: false
                )
                ...
                FinalScoreTeamRow(
                    ...
                    firstDowns: score.homeFirstDowns,
                    turnovers: score.homeTurnovers,
                    isHomeTeam: true
                )
            }
            .padding(.vertical, 9)
            ...
            HStack(alignment: .top, spacing: 10) {
                GameLeader(
                    decision: "W",
                    name: score.winningLeader,
                    line: score.winningLeaderLine,
                    color: Color(red: 0.94, green: 0.70, blue: 0.12)
                )
                GameLeader(
                    decision: "L",
                    name: score.losingLeader,
                    line: score.losingLeaderLine,
    ...
    private var scoreHeaders: some View {
        HStack(spacing: 4) {
            ForEach(["P", "1D", "TO"], id: \.self) { header in
                Text(header)
                    .font(.caption2.weight(.bold))
...
private struct PlayerStoryDiscoveryCardContent: View {
    let story: CollegeFootballPlayerStorySnapshot
    ...
    var body: some View {
        ZStack(alignment: .bottomLeading) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text("SEASON DEFINING  •  \(story.sourceName.uppercased())")
...
private struct DynamicStandingsDiscoveryCardContent: View {
        ...
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                Text(standings.teamName)
                    ...
                    .lineLimit(1)
                Text("\(standings.record)  •  \(standings.conferenceRecord) CONF")
    ...
    private var pointDifferentialSpotlight: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 10) {
                PointDifferentialMetric(
                    abbreviation: standings.teamAbbreviation,
                    value: standings.pointDifferential,
                    emphasized: true
                )
                ...
                PointDifferentialMetric(
                    ...
                    value: standings.comparisonPointDifferential,
                    emphasized: false
                )
            }

            Text("Conference benchmark: \(standings.comparisonTeam).")
    ...
    private var nextGameSpotlight: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            HStack(spacing: 7) {
                ...
                Text("\(standingsOrdinal(standings.nextOpponentConferencePosition)) \(standings.division)")
                    .font(.system(size: 7, weight: .bold))
                    .foregroundStyle(Color(white: 0.38))
            }

            Text("\(standings.nextGameVenue)  •  \(standings.broadcastNote)")
                .font(.system(size: 8, weight: .semibold))
                .foregroundStyle(Color(white: 0.32))
                .lineLimit(1)
        }
...
private struct PointDifferentialMetric: View {
...
private struct FinalScoreTeamRow: View {
    ...
    var body: some View {
        HStack(spacing: 4) {
            ...
            scoreValue(points)
            scoreValue(firstDowns)
            scoreValue(turnovers)
        }
    }
...
private struct GameLeader: View {
    let decision: String
    let name: String
    let line: String
    let color: Color
...
struct SearchLoadingCard: View {
    ...
    var body: some View {
        ...
        .accessibilityElement(children: .combine)
        .accessibilityIdentifier("collegeFootball-search-loading")
    }
}

private struct FeaturedResultCard: View {
    let module: CollegeFootballResultModule
    var compact = false
    ...
    private var genericCard: some View {
        ...
        .accessibilityElement(children: .combine)
        .accessibilityIdentifier("collegeFootball-featured-result")
    }
}

private struct PlayerGalleryResultCard: View {
    let player: CollegeFootballPlayerCard
    let leadImage: CollegeFootballPlayerGalleryImage

    private let buffsGold = BuffsTheme.brightGold
    ...
    var body: some View {
        ZStack(alignment: .bottomLeading) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                .padding(.vertical, 6)
                .background(buffsGold, in: Capsule())
            ...
            HStack {
                ...
                Image(systemName: "photo.stack.fill")
                    ...
                    .foregroundStyle(.white)
                    .frame(width: 30, height: 30)
                    .background(buffsGold, in: Circle())
            }
            .padding(13)
            .frame(maxHeight: .infinity, alignment: .top)
        }
        ...
        .overlay {
            RoundedRectangle(cornerRadius: 25, style: .continuous)
                .stroke(buffsGold.opacity(0.62), lineWidth: 1.5)
        }
        .shadow(color: buffsGold.opacity(0.22), radius: 16, y: 7)
        .accessibilityElement(children: .combine)
        .accessibilityIdentifier("collegeFootball-featured-result")
    }
}
...
struct SearchFailureCard: View {
    let failure: CollegeFootballSearchFailurePresentation
    ...
    var body: some View {
        ...
        .accessibilityIdentifier("collegeFootball-search-error")
    }
}
...
private extension CollegeFootballPlayerInsightKind {
    var eyebrow: String {
        switch self {
