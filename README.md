...
The out-of-box flow stays inside the Commish stage. A demo authentication toggle switches between a generic Commish-only signed-out stage and the personalized signed-in experience; the signed-out stage intentionally withholds search and personalized cards, but the Commish remains the product entry point. New users choose from all 30 MLB clubs, then choose a favorite player from the selected club’s active MLB roster; the Rockies roster has a local Hunter Goodman fallback for offline demos. Tapping the signed-out Commish begins personalization, while tapping the personalized Commish opens the resulting profile.
