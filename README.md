...
The signed-out experience is the Commish-led out-of-box flow itself: the generic Commish presents the team and player cards immediately, with no search or personalized discovery cards yet. New users choose from all 30 MLB clubs, then choose a favorite player from the selected club’s active MLB roster; the Rockies roster has a local Hunter Goodman fallback for offline demos. Finishing OOBE signs the user in and reveals the personalized home, while tapping the personalized Commish opens the resulting profile. A demo authentication toggle switches between the OOBE and personalized states.
